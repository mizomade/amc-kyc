  ...
  <div class="max-w-6xl mx-auto py-10 px-6 sm:px-10 bg-gray-50 min-h-screen">
    ...
    <div v-else-if="person" class="bg-white rounded-2xl shadow-lg p-10 space-y-10">
      ...
      <div class="text-center pt-6">
        <div v-if="updateSuccess" class="mb-4 text-green-600 font-semibold">
...
const updateSuccess = ref(false)  // ✅ Success message state

onMounted(async () => {
  const id = route.params.id
  try {
    const res = await $api.get('/search/person/', { params: { id } })
    if (res.data.length) {
...
async function saveChanges() {
  ...
  try {
    ...
    await $api.put(`/person/update/${person.value.id}`, payload)
    alert('Profile updated successfully!')
    console.log('Profile updated successfully!')   // ✅ Console success message
