...
class TreeNodeOut(Schema):
    ...
    siblings: Optional[List['TreeNodeOut']] = None
...
def build_full_tree(person, visited=None):
    ...
    # Spouse
    spouse_node = build_full_tree(person.spouse, visited) if person.spouse else None

    # Siblings
    siblings = []
    if person.father:
        sibling_qs = Person.objects.filter(father=person.father).exclude(id=person.id)
        for sibling in sibling_qs:
            if sibling.id not in visited:
                siblings.append(build_full_tree(sibling, visited))
